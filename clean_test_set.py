"""
? How to use:
? Run `python clean_test_set.py [test_set_path]`.
"""
...
1. Call `python clean_test.py [test_folder_path]` for each test lol
"""
...
for i in range(len(sub_dirs)):
    os.system('python clean_test.py ' + os.path.join(camp_path, sub_dirs[i]) + " " + str(i + 1) + " " + str(len(sub_dirs)))

# Validate that each test is clean
# for i in range(len(sub_dirs)):
    # os.system('python clean_test_check.py ' + os.path.join(camp_path, sub_dirs[i]) + " " + str(i + 1) + " " + str(len(sub_dirs)))